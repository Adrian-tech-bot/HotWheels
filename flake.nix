{
  ...
  let
    ...
    esp32Bluepad32IndexPatched =
      let
        raw = builtins.fromJSON (builtins.readFile (esp32-bluepad32-index + "/bluepad32_files/package_esp32_bluepad32_index.json"));
        patchedPackages = map (pkg: if pkg ? tools then pkg else pkg // { tools = []; }) raw.packages;
      in
      builtins.toFile "package_esp32_bluepad32_index.patched.json" (builtins.toJSON (raw // { packages = patchedPackages; }));
  ...
  (flake-utils.lib.eachDefaultSystem (system:
      ...
      rec {
        packages.arduino-cli =
          let
            bluepadPlatform =
              arduino-nix.latestVersion pkgs.arduinoPackages.platforms."esp32-bluepad32".esp32;
            hasBluepad32Library = pkgs.arduinoLibraries ? Bluepad32;
            bluepadLibrary =
              if hasBluepad32Library
              then arduino-nix.latestVersion pkgs.arduinoLibraries.Bluepad32
              else null;
          in
          pkgs.wrapArduinoCLI {
